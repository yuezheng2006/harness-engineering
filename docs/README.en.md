...
>
> Bringing in outside research no longer runs on vibes — it follows a pipeline frozen into a skill, [`curate-research`](https://github.com/yuezheng2006/harness-engineering/tree/main/.claude/skills/curate-research): review is automated by parallel agents (the feedback loop), `scripts/check-consistency.sh` keeps counts from drifting via C1–C8 (the mechanical rail), and whether something gets in is always a human gate (humans steer, agents execute).
>
