...
> 这个仓库开始策展自己了。
>
> 收录外部调研不再靠手感——它走一条固化成 skill 的流水线 [`curate-research`](https://github.com/yuezheng2006/harness-engineering/tree/main/.claude/skills/curate-research)：评审由并行 agent 自动完成（反馈回路），`scripts/check-consistency.sh` 的 C1–C8 守着计数不漂移（机械护栏），而"收不收进来"始终是一道人类闸门（人类掌舵、智能体执行）。
>
